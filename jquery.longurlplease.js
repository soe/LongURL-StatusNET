var longurlplease = {
  ...
  lengthen : function(options) {
    ...
    var lengthenShortUrl = longurlplease.urlStartExpandMethod;
    ...
    if (options.lengthenShortUrl !== null) {
      if (typeof options.lengthenShortUrl == 'function') {
        ...
        } else if (options.lengthenShortUrl == 'all') {
          lengthenShortUrl = longurlplease.urlAllExpandMethod;
        } else if (options.lengthenShortUrl == 'start') {
          lengthenShortUrl = longurlplease.urlStartExpandMethod;
        } else if (options.lengthenShortUrl == 'start-and-end') {
          lengthenShortUrl = longurlplease.urlStartEndExpandMethod;
        }
      }
    }
  ...
  fullExpandMethod : function(aTag, longUrl) {
    aTag.href = longUrl;
    aTag.innerHTML = longUrl.replace(/^http(s?):\/\//, '').replace(/^www\./, '');
  },
  ...
  urlAllExpandMethod : function(aTag, longUrl) {
    aTag.innerHTML = longUrl.replace(/^http(s?):\/\//, '').replace(/^www\./, '');
    aTag.title = longUrl;
  }, 
  urlStartExpandMethod : function(aTag, longUrl) {
    var linkText = longUrl.replace(/^http(s?):\/\//, '').replace(/^www\./, '');
    aTag.innerHTML = linkText.substring(0, 30) + '...';
    aTag.title = longUrl;
  }, 
  urlStartEndExpandMethod : function(aTag, longUrl) {
    var linkText = longUrl.replace(/^http(s?):\/\//, '').replace(/^www\./, '');
    aTag.innerHTML = linkText.substring(0,20) + '...' + longurl.substring(longurl.length-20, longurl.length );
    aTag.title = longUrl;
  },
